...
class BaseSupabaseRepository(BaseRepositoryImpl[T], Generic[T, M]):
    ...
    async def get_by_id(self, entity_id: str) -> Optional[T]:
        ...
        try:
            ...
            response = self._client.table(self._table_name).select("*").eq("id", entity_id).execute()
            ...
            self._logger.debug(f"Successfully retrieved {self._table_name} with ID: {entity_id}")
            return domain_model
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Failed to retrieve {self._table_name}: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Unexpected error retrieving {self._table_name}: {str(e)}") from e
    ...
    async def get_all(self, filters: Optional[Dict[str, Any]] = None) -> List[T]:
        ...
        """
        try:
            self._logger.debug(f"Fetching all {self._table_name} with filters: {filters}")
            ...
            self._logger.debug(f"Successfully retrieved {len(domain_models)} {self._table_name} records")
    ...
    async def create(self, entity: T) -> T:
        ...
        try:
            ...
            if not response.data:
                raise RepositoryError(f"Failed to create {self._table_name}: No data returned")
            ...
            self._logger.debug(f"Successfully created {self._table_name} with ID: {created_domain_model.id}")
            return created_domain_model
            
        except APIError as e:
            self._logger.error(f"Supabase API error creating {self._table_name}: {e}")
            if "duplicate key" in str(e).lower():
                raise ValidationError(f"{self._table_name} with this ID already exists") from e
            raise RepositoryError(f"Failed to create {self._table_name}: {str(e)}") from e
        except ValidationError:
            raise
        except Exception as e:
            self._logger.error(f"Unexpected error creating {self._table_name}: {e}")
            raise RepositoryError(f"Unexpected error creating {self._table_name}: {str(e)}") from e
    ...
    async def update(self, entity_id: str, entity: T) -> T:
        ...
        try:
            ...
            self._logger.debug(f"Updating {self._table_name} {entity_id} with data: {data}")
            
            response = self._client.table(self._table_name).update(data).eq("id", entity_id).execute()
            
            if not response.data:
                raise RepositoryError(f"Failed to update {self._table_name}: No data returned")
            ...
            self._logger.debug(f"Successfully updated {self._table_name} with ID: {entity_id}")
            return updated_domain_model
            
        except (NotFoundError, ValidationError):
            raise
        except APIError as e:
            self._logger.error(f"Supabase API error updating {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Failed to update {self._table_name}: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error updating {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Unexpected error updating {self._table_name}: {str(e)}") from e
    ...
    async def delete(self, entity_id: str) -> bool:
        ...
        try:
            ...
            response = self._client.table(self._table_name).delete().eq("id", entity_id).execute()
            
            # Check if any rows were affected
            deleted = len(response.data) > 0
            
            if deleted:
                self._logger.debug(f"Successfully deleted {self._table_name} with ID: {entity_id}")
            else:
                self._logger.debug(f"No {self._table_name} found to delete with ID: {entity_id}")
            
            return deleted
            
        except APIError as e:
            self._logger.error(f"Supabase API error deleting {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Failed to delete {self._table_name}: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error deleting {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Unexpected error deleting {self._table_name}: {str(e)}") from e
    ...
    async def exists(self, entity_id: str) -> bool:
        ...
        try:
            ...
            response = self._client.table(self._table_name).select("id").eq("id", entity_id).execute()
            
            return len(response.data) > 0
            
        except APIError as e:
            self._logger.error(f"Supabase API error checking existence of {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Failed to check {self._table_name} existence: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error checking existence of {self._table_name} {entity_id}: {e}")
            raise RepositoryError(f"Unexpected error checking {self._table_name} existence: {str(e)}") from e
    ...
    async def count(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to count {self._table_name}: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error counting {self._table_name}: {e}")
            raise RepositoryError(f"Unexpected error counting {self._table_name}: {str(e)}") from e
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "timeout" in str(e).lower() or "connection" in str(e).lower():
                    self._logger.warning(f"Query attempt {attempt + 1} failed, retrying: {e}")
                    continue
                raise