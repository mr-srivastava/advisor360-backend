...
class PartnerRepository(BaseSupabaseRepository[Partner, PartnerModel], IPartnerRepository):
    """
    Supabase implementation of the Partner repository.
    ...
    async def get_by_name(self, name: str) -> Optional[Partner]:
        ...
        try:
            ...
            self._logger.error(f"Supabase API error retrieving partner by name {name}: {e}")
            raise RepositoryError(f"Failed to retrieve partner by name: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving partner by name {name}: {e}")
            raise RepositoryError(f"Unexpected error retrieving partner by name: {str(e)}") from e
    ...
    async def get_by_entity_type(self, entity_type: EntityType) -> List[Partner]:
        ...
        try:
            ...
            self._logger.debug(f"Retrieved {len(partners)} partners for entity type {entity_type}")
            return partners
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving partners by entity type {entity_type}: {e}")
            raise RepositoryError(f"Failed to retrieve partners by entity type: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving partners by entity type {entity_type}: {e}")
            raise RepositoryError(f"Unexpected error retrieving partners by entity type: {str(e)}") from e
    ...
    async def search_by_name(self, search_term: str) -> List[Partner]:
        ...
        try:
            ...
            raise RepositoryError(f"Unexpected error searching partners by name: {str(e)}") from e
    ...
    async def get_active_partners(self) -> List[Partner]:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to retrieve active partners: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving active partners: {e}")
            raise RepositoryError(f"Unexpected error retrieving active partners: {str(e)}") from e
    ...
    async def get_partners_with_commissions(self) -> List[Partner]:
        ...
        try:
            ...
            self._logger.error(f"Supabase API error retrieving partners with commissions: {e}")
            raise RepositoryError(f"Failed to retrieve partners with commissions: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving partners with commissions: {e}")
            raise RepositoryError(f"Unexpected error retrieving partners with commissions: {str(e)}") from e
    ...
    async def get_partners_without_commissions(self) -> List[Partner]:
        ...
        try:
            ...
            commission_response = (self._client.table("entity_transactions")
                                 .select("entity_id")
                                 .execute())
            
            partner_ids_with_commissions = {row["entity_id"] for row in commission_response.data}
            
            # Get all partners
            all_partners_response = (self._client.table(self._table_name)
                                   .select("*, entity_types(name)")
                                   .order("name")
                                   .execute())
            ...
            self._logger.debug(f"Retrieved {len(partners)} partners without commissions")
            return partners
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving partners without commissions: {e}")
            raise RepositoryError(f"Failed to retrieve partners without commissions: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving partners without commissions: {e}")
            raise RepositoryError(f"Unexpected error retrieving partners without commissions: {str(e)}") from e
    ...
    async def get_entity_type_counts(self) -> Dict[EntityType, int]:
        ...
        try:
            ...
            for row in response.data:
                ...
                entity_type = PartnerModel._map_entity_type_name_to_enum(entity_type_name)
            ...
            self._logger.error(f"Supabase API error calculating entity type counts: {e}")
            raise RepositoryError(f"Failed to calculate entity type counts: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error calculating entity type counts: {e}")
            raise RepositoryError(f"Unexpected error calculating entity type counts: {str(e)}") from e
    ...
    async def name_exists(self, name: str, exclude_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            raise RepositoryError(f"Unexpected error checking name existence: {str(e)}") from e
    ...
    async def get_recently_created(self, limit: int = 10) -> List[Partner]:
        ...
        try:
            ...
            self._logger.error(f"Supabase API error retrieving recently created partners: {e}")
            raise RepositoryError(f"Failed to retrieve recently created partners: {str(e)}") from e
        except ValueError:
            raise
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving recently created partners: {e}")
            raise RepositoryError(f"Unexpected error retrieving recently created partners: {str(e)}") from e
    ...
    async def get_recently_updated(self, limit: int = 10) -> List[Partner]:
        ...
        try:
            ...
            self._logger.error(f"Supabase API error retrieving recently updated partners: {e}")
            raise RepositoryError(f"Failed to retrieve recently updated partners: {str(e)}") from e
        except ValueError:
            raise
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving recently updated partners: {e}")
            raise RepositoryError(f"Unexpected error retrieving recently updated partners: {str(e)}") from e
    ...
    async def get_all(self, filters: Optional[Dict[str, any]] = None) -> List[Partner]:
        ...
        try:
            ...
            raise RepositoryError(f"Unexpected error retrieving partners: {str(e)}") from e
    ...
    async def get_by_id(self, entity_id: str) -> Optional[Partner]:
        ...
        try:
            ...
            self._logger.error(f"Supabase API error retrieving partner {entity_id}: {e}")
            ...
            raise RepositoryError(f"Unexpected error retrieving partner: {str(e)}") from e