...
from ...core.exceptions.repository_exceptions import RepositoryError, ValidationError

logger = logging.getLogger(__name__)


class CommissionRepository(BaseSupabaseRepository[Commission, CommissionModel], ICommissionRepository):
    """
    Supabase implementation of the Commission repository.
    ...
    async def get_by_partner_id(self, partner_id: str) -> List[Commission]:
        ...
        try:
            ...
            self._logger.debug(f"Retrieved {len(commissions)} commissions for partner {partner_id}")
            return commissions
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving commissions for partner {partner_id}: {e}")
            raise RepositoryError(f"Failed to retrieve commissions for partner: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving commissions for partner {partner_id}: {e}")
            raise RepositoryError(f"Unexpected error retrieving commissions for partner: {str(e)}") from e
    ...
    async def get_by_financial_year(self, financial_year: FinancialYear) -> List[Commission]:
        ...
        try:
            ...
            self._logger.debug(f"Fetching commissions for financial year: {financial_year}")
            ...
            self._logger.debug(f"Retrieved {len(commissions)} commissions for financial year {financial_year}")
            return commissions
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving commissions for FY {financial_year}: {e}")
            raise RepositoryError(f"Failed to retrieve commissions for financial year: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving commissions for FY {financial_year}: {e}")
            raise RepositoryError(f"Unexpected error retrieving commissions for financial year: {str(e)}") from e
    ...
    async def get_by_partner_and_financial_year(self, partner_id: str, 
        ...
        try:
            ...
            self._logger.debug(f"Fetching commissions for partner {partner_id} in FY {financial_year}")
            ...
            self._logger.debug(f"Retrieved {len(commissions)} commissions for partner {partner_id} in FY {financial_year}")
            return commissions
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving commissions for partner {partner_id} in FY {financial_year}: {e}")
            raise RepositoryError(f"Failed to retrieve commissions for partner and financial year: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving commissions for partner {partner_id} in FY {financial_year}: {e}")
            raise RepositoryError(f"Unexpected error retrieving commissions for partner and financial year: {str(e)}") from e
    ...
    async def get_by_date_range(self, start_date: date, end_date: date) -> List[Commission]:
        ...
        try:
            ...
            self._logger.debug(f"Retrieved {len(commissions)} commissions in date range")
            return commissions
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving commissions for date range: {e}")
            raise RepositoryError(f"Failed to retrieve commissions for date range: {str(e)}") from e
        except ValueError:
            raise
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving commissions for date range: {e}")
            raise RepositoryError(f"Unexpected error retrieving commissions for date range: {str(e)}") from e
    ...
    async def get_by_month_year(self, month: int, year: int) -> List[Commission]:
        ...
        try:
            ...
            self._logger.debug(f"Retrieved {len(commissions)} commissions for {month}/{year}")
            return commissions
            
        except APIError as e:
            self._logger.error(f"Supabase API error retrieving commissions for {month}/{year}: {e}")
            raise RepositoryError(f"Failed to retrieve commissions for month/year: {str(e)}") from e
        except ValueError:
            raise
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving commissions for {month}/{year}: {e}")
            raise RepositoryError(f"Unexpected error retrieving commissions for month/year: {str(e)}") from e
    ...
    async def get_total_amount_by_partner(self, partner_id: str, 
        ...
        try:
            ...
            if financial_year:
                start_date = financial_year.start_date
                end_date = financial_year.end_date
                query = query.gte("month", start_date.isoformat()).lte("month", end_date.isoformat())
            ...
            self._logger.error(f"Supabase API error calculating total for partner {partner_id}: {e}")
            raise RepositoryError(f"Failed to calculate total for partner: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error calculating total for partner {partner_id}: {e}")
            raise RepositoryError(f"Unexpected error calculating total for partner: {str(e)}") from e
    ...
    async def get_total_amount_by_financial_year(self, financial_year: FinancialYear) -> float:
        ...
        try:
            ...
            self._logger.debug(f"Calculating total amount for financial year {financial_year}")
            ...
            self._logger.debug(f"Total amount for financial year {financial_year}: {total}")
            return total
            
        except APIError as e:
            self._logger.error(f"Supabase API error calculating total for FY {financial_year}: {e}")
            raise RepositoryError(f"Failed to calculate total for financial year: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error calculating total for FY {financial_year}: {e}")
            raise RepositoryError(f"Unexpected error calculating total for financial year: {str(e)}") from e
    ...
    async def get_monthly_totals(self, financial_year: FinancialYear) -> Dict[str, float]:
        ...
        try:
            ...
            self._logger.debug(f"Calculating monthly totals for financial year {financial_year}")
            ...
            self._logger.debug(f"Monthly totals for FY {financial_year}: {monthly_totals}")
            return monthly_totals
            
        except APIError as e:
            self._logger.error(f"Supabase API error calculating monthly totals for FY {financial_year}: {e}")
            raise RepositoryError(f"Failed to calculate monthly totals: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error calculating monthly totals for FY {financial_year}: {e}")
            raise RepositoryError(f"Unexpected error calculating monthly totals: {str(e)}") from e
    ...
    async def get_partner_totals(self, financial_year: Optional[FinancialYear] = None) -> Dict[str, float]:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to calculate partner totals: {str(e)}") from e
        except Exception as e:
            self._logger.error(f"Unexpected error calculating partner totals: {e}")
            raise RepositoryError(f"Unexpected error calculating partner totals: {str(e)}") from e
    ...
    async def get_recent_commissions(self, limit: int = 10) -> List[Commission]:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to retrieve recent commissions: {str(e)}") from e
        except ValueError:
            raise
        except Exception as e:
            self._logger.error(f"Unexpected error retrieving recent commissions: {e}")
            raise RepositoryError(f"Unexpected error retrieving recent commissions: {str(e)}") from e
    ...
    async def search_by_description(self, search_term: str) -> List[Commission]:
        ...
        try:
            ...
            self._logger.debug(f"Found {len(commissions)} commissions matching description search")
            return commissions
            
        except APIError as e:
            self._logger.error(f"Supabase API error searching commissions by description: {e}")
            raise RepositoryError(f"Failed to search commissions by description: {str(e)}") from e
        except ValueError:
            raise
        except Exception as e:
            self._logger.error(f"Unexpected error searching commissions by description: {e}")
            raise RepositoryError(f"Unexpected error searching commissions by description: {str(e)}") from e