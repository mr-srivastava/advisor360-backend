...
class CommissionModel(BaseModel):
    ...
    entity_id: str = Field(..., description="Partner/Entity ID (maps to partner_id in domain)")
    ...
    description: Optional[str] = Field(None, max_length=500, description="Commission description")
    ...
    def from_domain(cls, commission: Commission) -> "CommissionModel":
        ...
        month_datetime = datetime.combine(
            commission.transaction_date.replace(day=1),
            datetime.min.time()
        )
