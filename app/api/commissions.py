...
from ..core.exceptions import CommissionNotFound, PartnerNotFound, DomainException
...
async def list_commissions(
    ...
    financial_year: Optional[str] = Query(None, description="Filter by financial year (e.g., FY24-25)")
):
    """Get all commissions with optional filtering"""
    try:
        if partner_id:
            commissions = await commission_service.get_commissions_by_partner(partner_id)
        elif financial_year:
            commissions = await commission_service.get_commissions_by_financial_year(financial_year)
        else:
