...
from app.domain.partner import Partner, EntityType
...
class DomainToDTOMapper:
    ...
    # Commission mappings
    @staticmethod
    def commission_to_response(commission: Commission, partner: Optional[Partner] = None) -> CommissionResponse:
    ...
    @staticmethod
    def transaction_to_response(transaction: Transaction, partner: Optional[Partner] = None) -> TransactionResponse:
...
class AnalyticsMapper:
    ...
    def create_monthly_commission_data(
        ...
        for commission in commissions:
            month_key = f"{commission.get_year()}-{commission.transaction_date.month:02d}"
        ...
        for i, key in enumerate(sorted_keys):
            ...
            if include_growth and i > 0:
                ...
                if prev_total > 0:
                    growth_percentage = ((data['total_amount'] - prev_total) / prev_total) * 100
            
            result.append(MonthlyCommissionData(
                month=data['month'],
                year=data['year'],
    ...
    def create_partner_commission_data(
        ...
        for partner_id, data in partner_data.items():
            ...
            result.append(PartnerCommissionData(
    ...
    def create_partner_performance_data(
        ...
        for commission in commissions:
            if commission.partner_id in partner_performance:
                ...
                if (data['last_commission_date'] is None or 
                    commission.created_at > data['last_commission_date']):
        ...
        for rank, (partner_id, data) in enumerate(sorted_partners, 1):
            ...
            result.append(PartnerPerformanceData(
                partner_id=partner_id,
                partner_name=partner.name,
                entity_type=partner.entity_type.value,
                total_commissions=total_commissions,
                commission_count=commission_count,
                average_commission=total_commissions / commission_count if commission_count > 0 else 0.0,
...
class ValidationMapper:
    ...
    @staticmethod
    def validation_error_to_dto(field: str, message: str, value: Any = None) -> Dict[str, Any]:
...
def map_commission_with_partner(commission: Commission, partners: List[Partner]) -> CommissionResponse:
...
def map_transaction_with_partner(transaction: Transaction, partners: List[Partner]) -> TransactionResponse:
