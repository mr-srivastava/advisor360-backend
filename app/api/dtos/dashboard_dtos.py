...
from .common_dtos import DataResponse
...
class DashboardQueryParams(BaseModel):
    ...
    financial_year: Optional[str] = Field(
        None, description="Financial year for metrics (e.g., FY24-25)"
    )
    include_trends: bool = Field(True, description="Include trend data")
    months_back: int = Field(
        6, description="Number of months to look back", ge=1, le=24
    )
...
class PerformanceMetricsData(BaseModel):
    ...
    monthly_growth: list[MonthlyGrowthData] = Field(
        ..., description="Monthly growth data"
    )
    entity_performance: list[EntityPerformanceData] = Field(
...
class RecentActivityData(BaseModel):
    ...
    recent_commissions: list[RecentCommissionActivity] = Field(
        ..., description="Recent commission records"
    )
    monthly_commissions: list[MonthlyCommissionSummary] = Field(
...
class QuarterlyData(BaseModel):
    ...
    growth: Optional[float] = Field(
        None, description="Growth compared to previous quarter"
    )
...
class YearlyComparisonData(BaseModel):
    ...
    count: int = Field(..., description="Number of transactions")
    growth: Optional[float] = Field(
        None, description="Growth compared to previous year"
    )
...
class DashboardAnalyticsData(BaseModel):
    ...
    yearly_comparison: list[YearlyComparisonData] = Field(
        ..., description="Year-over-year comparison"
    )
    top_performers: list[EntityPerformanceData] = Field(
        ..., description="Top performing entities"
    )
...
class DashboardMapper:
    ...
    def create_stat_card(
        ...
        """Create a stat card."""
        return StatCard(
            id=id, title=title, value=value, subtitle=subtitle, icon=icon, trend=trend
        )
