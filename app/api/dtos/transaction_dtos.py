...
class TransactionResponse(BaseModel):
    ...
    @classmethod
    def from_domain(cls, transaction: Transaction, partner: Optional[Partner] = None) -> "TransactionResponse":
...
class TransactionListResponse(ListResponse[TransactionResponse]):
    ...
    summary: Optional[TransactionSummary] = Field(None, description="Summary statistics")
...
class TransactionAnalyticsResponse(BaseResponse):
    ...
    monthly_data: List[MonthlyTransactionData] = Field(..., description="Monthly breakdown")
    partner_data: List[PartnerTransactionData] = Field(..., description="Partner-wise breakdown")
...
class TransactionMapper:
    ...
    @staticmethod
    def to_response(transaction: Transaction, partner: Optional[Partner] = None) -> TransactionResponse:
        """Convert domain model to response DTO."""
        return TransactionResponse.from_domain(transaction, partner)
    
    @staticmethod
    def to_response_list(transactions: List[Transaction], partners: Optional[List[Partner]] = None) -> List[TransactionResponse]:
        """Convert list of domain models to response DTOs."""
        partner_map = {p.id: p for p in partners} if partners else {}
        return [
            TransactionResponse.from_domain(transaction, partner_map.get(transaction.partner_id))
            for transaction in transactions
        ]
    
    @staticmethod
    def create_summary(transactions: List[Transaction], financial_year: Optional[str] = None) -> TransactionSummary:
        ...
        return TransactionSummary(
            ...
            financial_year=financial_year or transactions[0].financial_year.to_string("short")
        )