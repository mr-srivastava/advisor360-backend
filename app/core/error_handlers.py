...
from .logging.error_logger import error_logger, error_context
...
async def advisor360_exception_handler(request: Request, exc: DomainException) -> JSONResponse:
    """Handle legacy Advisor360 exceptions (backward compatibility)."""
    return await domain_exception_handler(request, exc)


async def financial_year_not_found_handler(request: Request, exc: FinancialYearError) -> JSONResponse:
    """Handle legacy financial year not found exceptions (backward compatibility)."""
    return await domain_exception_handler(request, exc)


async def partner_not_found_handler(request: Request, exc: PartnerNotFound) -> JSONResponse:
    """Handle legacy partner not found exceptions (backward compatibility)."""
    return await domain_exception_handler(request, exc)


async def commission_not_found_handler(request: Request, exc: CommissionNotFound) -> JSONResponse:
    """Handle legacy commission not found exceptions (backward compatibility)."""
    return await domain_exception_handler(request, exc)


async def invalid_financial_year_handler(request: Request, exc: ValidationError) -> JSONResponse:
