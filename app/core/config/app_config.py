...
class AppConfig(BaseSettings):
    ...
    app_name: str = Field(
        default="Commission Tracker API", description="Application name"
    )
    ...
    database_timeout: int = Field(
        default=30, env="DATABASE_TIMEOUT", description="Database timeout"
    )
    ...
    log_file: Optional[str] = Field(
        default=None, env="LOG_FILE", description="Log file path"
    )
    ...
    def database(self) -> DatabaseConfig:
        ...
        return DatabaseConfig(
            url=self.database_url, key=self.database_key, timeout=self.database_timeout
        )
    ...
    def logging(self) -> LoggingConfig:
        ...
        return LoggingConfig(level=self.log_level, file_path=self.log_file)
