...
from .structured_logger import clear_request_context, get_logger, set_request_context
...
class RequestLogger:
    ...
    def log_request_end(
        ...
        if response_body and (
            status_code >= 400 or self.logger.logger.isEnabledFor(10)
        ):  # DEBUG level
        ...
        if status_code >= 500:
            self.logger.error(
                f"Request failed: {method} {path} - {status_code}", log_data
            )
        elif status_code >= 400:
            self.logger.warning(
                f"Request error: {method} {path} - {status_code}", log_data
            )
        else:
            self.logger.info(
                f"Request completed: {method} {path} - {status_code}", log_data
            )
    ...
    def log_request_metrics(
        ...
        self.logger.info(
            f"Request metrics for {time_window}: {total_requests} requests", log_data
        )
