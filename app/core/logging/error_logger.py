...
class StructuredErrorLogger:
            ...
            log_data["error"].update(
                {
                    ...
                    "original_error": (
                        str(getattr(exception, "original_error", None))
                        if getattr(exception, "original_error", None)
                        else None
                    ),
                }
            )
    ...
    def log_error(
        ...
        if severity == "ERROR":
            if (
                "ValidationError" in exception.__class__.__name__
                or "NotFoundError" in exception.__class__.__name__
            ):
                severity = "WARNING"
...
def error_context(
    ...
    correlation_token = (
        correlation_id_context.set(correlation_id) if correlation_id else None
    )
