...
class ValidationErrorResponse(ErrorResponse):
    ...
    error_code: str = Field("VALIDATION_ERROR", description="Always VALIDATION_ERROR for validation errors")
    field_errors: Optional[Dict[str, List[str]]] = Field(None, description="Field-specific error messages")
...
class NotFoundErrorResponse(ErrorResponse):
    ...
    error_code: str = Field("NOT_FOUND", description="Always NOT_FOUND for not found errors")
    entity_type: Optional[str] = Field(None, description="Type of entity that was not found")
    entity_id: Optional[str] = Field(None, description="ID of entity that was not found")
...
class BusinessRuleErrorResponse(ErrorResponse):
    ...
    error_code: str = Field("BUSINESS_RULE_VIOLATION", description="Always BUSINESS_RULE_VIOLATION")
    rule_name: Optional[str] = Field(None, description="Name of the violated business rule")
    rule_description: Optional[str] = Field(None, description="Description of the business rule")
...
class InternalServerErrorResponse(ErrorResponse):
    ...
    error_code: str = Field("INTERNAL_SERVER_ERROR", description="Always INTERNAL_SERVER_ERROR")
    message: str = Field("An internal server error occurred", description="Generic error message")
    
    # Override sensitive fields to not expose internal details
    context: Optional[Dict[str, Any]] = Field(None, description="Limited context for security")
    ...
    if isinstance(exception, ValidationError):
        return ValidationErrorResponse(
            ...
            details=[ErrorDetail(
                ...
                value=str(exception.value) if exception.value is not None else None
            )] if exception.field else None,
            **base_kwargs
        )
