...
from ..logging.error_logger import error_context, error_logger, log_exception
...
def create_commission(
    partner_id: str, amount: float, description: Optional[str] = None
):
...
def example_service_with_logging():
    ...
    try:
        ...
        error_logger.log_validation_error(
            e, field=e.field, value=e.value, validation_rule=e.validation_rule
        )
        ...
        error_logger.log_business_rule_violation(
            e, rule_name=e.rule_name, entity_type="Partner", entity_id=partner_id
        )
        raise
