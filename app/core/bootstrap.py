...
def create_supabase_client() -> Client:
    ...
    client = create_client(
        ...
        supabase_key=config.database_key
    )
...
def validate_dependencies(container: Container) -> None:
    ...
    for service_interface in required_services:
        if not container.is_registered(service_interface):
            raise ValueError(f"Required service {service_interface.__name__} is not registered")
        
        # Try to resolve the service
        try:
            service = container.get(service_interface)
            logger.debug(f"Successfully resolved {service_interface.__name__}: {type(service).__name__}")
        except Exception as e:
            raise ValueError(f"Failed to resolve {service_interface.__name__}: {str(e)}")
    
    # Check repositories
    for repo_interface in required_repositories:
        if not container.is_registered(repo_interface):
            raise ValueError(f"Required repository {repo_interface.__name__} is not registered")
        
        # Try to resolve the repository
        try:
            repo = container.get(repo_interface)
            logger.debug(f"Successfully resolved {repo_interface.__name__}: {type(repo).__name__}")
        except Exception as e:
