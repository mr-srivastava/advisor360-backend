...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def _read_request_body(self, request: Request) -> Optional[bytes]:
        ...
        try:
            ...
            return (
                body if len(body) <= self.max_body_size else body[: self.max_body_size]
            )
        except Exception:
            return None
    ...
    def _extract_request_data(
        self, request: Request, body: Optional[bytes] = None
    ) -> dict[str, Any]:
    ...
    def _extract_response_data(
        self, response: Response, body: Optional[bytes] = None
    ) -> dict[str, Any]:
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            self.request_logger.log_request_end(
                ...
                response_headers=(
                    response_data.get("headers") if self.log_headers else None
                ),
...
class ResponseBodyLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        should_capture = response.status_code >= 400 or getattr(
            request.state, "log_response_body", False
        )
